...
@testset "Aqua.jl" begin
    ...
    Aqua.test_all(PixelFlipper; ambiguities = false)
    Aqua.test_ambiguities(PixelFlipper)
end

@testset "ExplicitImports.jl" begin
    @info "Running ExplicitImports.jl import-hygiene tests."
    @test check_no_stale_explicit_imports(PixelFlipper) === nothing
    @test check_all_explicit_imports_via_owners(PixelFlipper) === nothing
    @test check_all_qualified_accesses_via_owners(PixelFlipper) === nothing
